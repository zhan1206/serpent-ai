...
class TestTokenOptimizer:
    ...
    async def test_calculate_tokens(self):
        ...
        # 测试记录请求（实际方法）
        optimizer.record_request(100, 50, 0.01)
    ...
    async def test_optimize_prompt(self):
        ...
        # 原始提示词
        original = "你是一个有用的AI助手。你应该提供准确、有帮助的回答。"
...
你总是在回答中保持友好和专业。"""
    ...
    async def test_cache(self):
        ...
        # 第一次蒸馏
        result1 = distiller.distill("测试1")

        # 第二次相同内容应该从缓存获取
        result2 = distiller.distill("测试1")
...
class TestIncrementalContextManager:
    ...
    async def test_get_incremental(self):
        ...
        # 获取增量
        incremental = manager.get_incremental_messages("test_session", messages)
    ...
    async def test_save_state(self):
        ...
        # 保存状态
        state = manager.save_state("test_session", messages)
    ...
    async def test_restore_state(self):
        ...
        # 恢复状态
        restored = manager.get_state("test_session")
...
class TestMultiLevelCache:
    ...
    async def test_prompt_cache(self):
        ...
        # 设置缓存
        cache.set("prompt", "test_key", "优化后的提示词")

        # 获取缓存
        result = cache.get("prompt", "test_key")
    ...
    async def test_model_response_cache(self):
        ...
        # 设置缓存
        key = str([{"role": "user", "content": "你好"}, {"role": "user", "content": "今天天气怎么样"}])
        response = {"content": "今天天气很好", "model": "gpt-3.5-turbo"}

        cache.set("model_response", key, response)

        # 获取缓存
        result = cache.get("model_response", key)
    ...
    async def test_tool_cache(self):
        ...
        # 设置缓存
        cache.set("tool", "web_search", {"id": "ws001", "description": "搜索"})

        # 获取
        result = cache.get("tool", "web_search")
